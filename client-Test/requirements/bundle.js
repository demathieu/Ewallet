...
function filterCombinator(type, that, state, chainOperator, defaultState, target, name, argscalled, recv, inputObject) {
  ...
  if (filterChain(chainOperator)(that.state, target, name, argscalled, recv, inputObject['arguments']|| [], inputObject)) {
    if (type === 'function') {
      console.log( 'deze hier 1')
      //return target[name].apply(target, argscalled);
      //return Reflect.get(that,name,recv) ;
      return Reflect.apply(target[name],that,argscalled);
    } else {
      console.log( 'deze hier 2')
...
},{"./helper.js":1,"./reflect.js":3}],3:[function(require,module,exports){
(function (global){
